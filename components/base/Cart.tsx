...
const Cart = () => {
    ...
    try {
      const swapRequests = [
        ...buyTokens.map((coin) => ({
          ...
          tokenOut: coin.detail_platforms.base.contract_address,
        ...
        ...sellTokens.map((coin) => ({
          tokenIn: coin.detail_platforms.base.contract_address,
    ...
    try {
      ...
      const sellApprove = sellTokens.map((token, index) => {
        const approvalData = encodeFunctionData({
          ...
          functionName: "approve",
          args: [swapData.swapResponses[index].to, parseUnits(String(token.amount), Number(token.decimal_place))],
        });

        return {
          to: token.detail_platforms.base.contract_address,
          data: approvalData,
          value: BigInt(0),
        };
      });
  ...
  return (
    <>
      <div
        onScrollCapture={() => setShowCart(false)}
      ...
      <div
        ...
        } fixed top-[60px] right-0 p-5 overflow-hidden bg-zinc-950 w-screen h-[calc(100vh-60px)] sm:w-4/12 flex flex-col gap-2 z-[10] shadow-2xl shadow-zinc-700`}
      >
        ...
        <div className="flex-grow overflow-y-auto hide_scrollbar">
              ...
              <div className="flex flex-col mb-2 text-zinc-300 mt-2">
                ...
                <div className="w-full relative">
                  <input
                    type="text"
                    inputMode="numeric"
                    className="bg-zinc-900 rounded-lg px-4 py-2 w-full text-zinc-200 text-base outline-none"
                ...
                <span className="ml-auto text-xs text-cyan-500 mb-1 pr-1">
                  Balance: {FormatDecimalValue(Number(usdcBalance?.formatted))} USDC
                </span>
              </div>
            ) : (
              <>
          ...
          <div className="border border-zinc-700 p-3 rounded-xl bg-opacity-50">
            ...
            {sellTokens.map((coin) => (
              <SelectedSellToken key={coin.id} coin={coin} swapAmounts={swapAmounts} />
            ))}
          </div>
        </div>
          ...
          <div className="bg-zinc-900 rounded-lg p-3">
            ...
            <div className="flex justify-between items-center">
              {(buyTokens.length !== 0 || sellTokens.length !== 0) && (
                <button
                  onClick={
                    buttonState === "proceed" ? handleProceed : buttonState === "rebalance" ? toggleReview : undefined
                  }
                  className={`flex items-center justify-center gap-2 w-full px-5 py-3 bg-zinc-800 text-white rounded-lg transition-all duration-300 mb-2
    ...
    ${
      isLoading || buttonState === "quoting" || buttonState === "rebalancing" ? "cursor-not-allowed opacity-50" : ""
    } text-base font-bold text-center`} // Center the button text
                  disabled={
                    isLoading ||
                    buttonState === "quoting" ||
                ...
                >
                  {(isLoading || buttonState === "quoting" || buttonState === "rebalancing") && <Loader />}
                  {buttonState === "proceed" && (
                    <>

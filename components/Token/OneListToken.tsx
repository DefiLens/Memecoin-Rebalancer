...
const OneToken: React.FC<OneTokenProps> = ({ token, index, action }: any) => {
  ...
  return (
    <tr
      ...
      data-index={index}
    >
      <td className="bg-zinc-950 px-4 py-1 font-normal transition-all ease-in-out group-hover:bg-zinc-900 md:py-2 whitespace-nowrap border-b border-zinc-800/80 first:border-l last:border-r sm:sticky left-0 z-10">
        <div className="flex items-center">
          ...
          <div>
            <div className="text-sm font-semibold text-zinc-100 flex items-center gap-1">
              ...
              {token?.detail_platforms?.base?.contract_address && (
                <CopyButton
                  copy={token?.detail_platforms?.base?.contract_address}
                  className="text-xs"
                  showToast={false}
                />
              )}
      ...
      <td className="bg-zinc-950 px-4 py-1 font-normal transition-all ease-in-out group-hover:bg-zinc-900 md:py-2 whitespace-nowrap border-b border-zinc-800/80 first:border-l last:border-r text-right">
        <span>${FormatDecimalValue(Number(token.current_price)) ?? "N/A"}</span>
      </td>
      {token.balance && (
        <td
          className="bg-zinc-950 px-4 py-1 font-normal transition-all ease-in-out group-hover:bg-zinc-900 md:py-2 whitespace-nowrap border-b border-zinc-800/80 first:border-l last:border-r text-right tabular-nums"
          colSpan={1}
        >
      ...
      <td
        className="bg-zinc-950 px-4 py-1 font-normal transition-all ease-in-out group-hover:bg-zinc-900 md:py-2 whitespace-nowrap border-b border-zinc-800/80 first:border-l last:border-r text-right min-w-24 tabular-nums"
        colSpan={1}
      >
      ...
      >
        <span className="text-buy">
          ...
          >
            {token.price_change_percentage_24h != null ? `${token.price_change_percentage_24h.toFixed(2)}%` : "N/A"}
          </span>
        </span>
      </td>
      <td
